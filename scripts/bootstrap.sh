#!/usr/bin/env bash
# Install claude-sdlc in the repository of the current directory.
#
#   bash <(curl -sSL https://raw.githubusercontent.com/jmformenti/claude-sdlc/main/scripts/bootstrap.sh) \
...
TEMPLATE_REPO="jmformenti/claude-sdlc"
reviewer=""
language="en"
ref="v1"
with_e2e=false
...
echo "Installing claude-sdlc@$ref into $repo (reviewer: ${reviewer:-none}, language: $language, e2e: $with_e2e)"
...
copy .github/workflows/claude-sdlc.yml
copy .github/workflows/claude-mention.yml
copy .github/claude-sdlc/implement.md
copy .github/claude-sdlc/review.md
copy .claude/commands/plan-issue.md
if $with_e2e; then
  copy .github/claude-sdlc/e2e.md
  copy .github/claude-sdlc/e2e-up.sh
  copy .github/claude-sdlc/e2e-down.sh
fi

# --- fill in the caller ------------------------------------------------------
caller=.github/workflows/claude-sdlc.yml
if ! $dry_run && [ -f "$caller" ]; then
  ...
  rm -f "$caller.bak"
  sed -i.bak -e "s|@v1$|@$ref|" .github/workflows/claude-mention.yml && rm -f .github/workflows/claude-mention.yml.bak
fi
...
Done. Next steps:
  ...
  2. Edit .github/claude-sdlc/*.md with your project's checklist (or delete them).
$( $with_e2e && echo "  3. Adapt .github/claude-sdlc/e2e-up.sh / e2e-down.sh to start your app." )
